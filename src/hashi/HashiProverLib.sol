...
library HashiProverLib {
     ...
     */
    function verifyForeignStorage(AccountAndStorageProof calldata proof, address shoyuBashi)
        internal
        view
        ...
        for (uint256 i = 0; i < ancestralBlockHeaders.length; i++) {
            ...
            if (ancestralBlockHeaderHash != currentBlockHeaderHash) {
                revert ConflictingBlockHeader(
                    currentAncestralBlockNumber, ancestralBlockHeaderHash, currentBlockHeaderHash
                );
            }
    ...
    function extractReceiptFields(bytes memory value) internal pure returns (RLPReader.RLPItem[] memory) {
        ...
        uint256 offset;
        if (
            txTypeOrFirstByte == 0x01 || txTypeOrFirstByte == 0x02 || txTypeOrFirstByte == 0x03
        ...
        return RLPReader.RLPItem({length: value.length - offset, ptr: RLPReader.MemoryPointer.wrap(ptr + offset)})
            .readList();
    }
     ...
     */
    function verifyAccountProof(address account, bytes32 stateRoot, bytes[] memory proof)
        internal
        pure
     ...
     */
    function verifyStorageProof(bytes32 storageHash, bytes32[] memory storageKeys, bytes[][] memory proof)
        internal
        pure
        returns (bytes[] memory)
    {
        ...
        for (uint256 i = 0; i < proof.length;) {
            RLPReader.RLPItem memory item =
                RLPReader.toRLPItem(SecureMerkleTrie.get(abi.encode(storageKeys[i]), proof[i], storageHash));
