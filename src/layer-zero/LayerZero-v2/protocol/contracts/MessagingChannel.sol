...
abstract contract MessagingChannel is IMessagingChannel {
    ...
    function nilify(
        ...
        if (curPayloadHash != _payloadHash)
            revert Errors.LZ_PayloadHashNotFound(curPayloadHash, _payloadHash);
        if (
            _nonce <= lazyInboundNonce[_oapp][_srcEid][_sender] &&
    ...
    function _clearPayload(
        ...
        if (expectedHash != actualHash)
            revert Errors.LZ_PayloadHashNotFound(expectedHash, actualHash);
