...
abstract contract MessageLibManager is Ownable, IMessageLibManager {
    ...
    modifier isSendLib(address _lib) {
        if (_lib != DEFAULT_LIB) {
            if (IMessageLib(_lib).messageLibType() == MessageLibType.Receive)
                revert Errors.LZ_OnlySendLib();
        }
        _;
    }
    ...
    modifier isReceiveLib(address _lib) {
        if (_lib != DEFAULT_LIB) {
            if (IMessageLib(_lib).messageLibType() == MessageLibType.Send)
                revert Errors.LZ_OnlyReceiveLib();
        }
        _;
    }

    modifier onlyRegisteredOrDefault(address _lib) {
        if (!isRegisteredLibrary[_lib] && _lib != DEFAULT_LIB)
            revert Errors.LZ_OnlyRegisteredOrDefaultLib();
        _;
    }
    ...
    function registerLibrary(address _lib) public onlyOwner {
        ...
        if (!IERC165(_lib).supportsInterface(type(IMessageLib).interfaceId))
            revert Errors.LZ_UnsupportedInterface();
        ...
        onlySupportedEid(_newLib, _eid)
    {
        ...
        if (_gracePeriod > 0) {
            ...
            if (oldLib == DEFAULT_LIB || _newLib == DEFAULT_LIB)
                revert Errors.LZ_OnlyNonDefaultLib();
